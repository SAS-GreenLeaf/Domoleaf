...
intern int pth_attr_ctrl(int cmd, pth_attr_t a, int op, va_list ap)
{
    ...
    switch (op) {
        ...
        case PTH_ATTR_TIME_SPAWN: {
            ...
            if (a->a_tid != NULL)
                pth_time_set(&dst1, &a->a_tid->spawned);
            else
                pth_time_set(&dst1, PTH_TIME_ZERO);
	    pth_itime_to_time(dst, &dst1);
            break;
        }
        case PTH_ATTR_TIME_LAST: {
            ...
            dst = va_arg(ap, pth_time_t *);
            if (a->a_tid != NULL)
                pth_time_set(&dst1, &a->a_tid->lastran);
            else
                pth_time_set(&dst1, PTH_TIME_ZERO);
	    pth_itime_to_time(dst, &dst1);
            break;
        ...
        case PTH_ATTR_TIME_RAN: {
            ...
            dst = va_arg(ap, pth_time_t *);
            if (a->a_tid != NULL)
                pth_time_set(&dst1, &a->a_tid->running);
            else
                pth_time_set(&dst1, PTH_TIME_ZERO);
	    pth_itime_to_time_iv(dst, &dst1);
            break;
        }
