...
static pth_itime_t   pth_loadticknext;
static pth_itime_t   pth_loadtickgap = PTH_TIME(1,0);
 ...
 */
intern void pth_sched_eventmanager(pth_itime_t *now, int dopoll)
{
    pth_t nexttimer_thread;
    pth_event_t nexttimer_ev;
    pth_itime_t nexttimer_value;
    pth_itime_t tmp;
    pth_event_t evh;
    pth_event_t ev;
    pth_t t;
    ...
    if (dopoll) {
        ...
        pth_time_set(&tmp, &nexttimer_value);
        pth_time_sub(&tmp, now);
	delay.tv_sec = tmp.t_sec;
	delay.tv_usec = tmp.t_usec;
        pdelay = &delay;
    }
    else {
